...
class DataService{
    ...
    async createEntity(Typed, doc){
        let conn=await this.collection(Typed)
        try{
	...
	async updateEntity(Typed,query,doc){
		let conn=await this.collection(Typed)
        try{
    ...
    async patchEntity(Typed, query, $set){
        let conn=await this.collection(Typed)
        try{
    ...
    async remove1Entity(Typed, query){
        let conn=await this.collection(Typed)
        try{
            ...
            require("./file").resolver(this).Mutation
                .file_clean(null,{host:`${Typed}:${_id}`},{app:this})
    ...
    async removeEntity(Typed, query){
        let conn=await this.collection(Typed)
        try{
            ...
            removing.forEach(({_id})=>{
                require("./file").resolver.Mutation
                    .file_clean(null,{host:`${Typed}:${_id}`},ctx)
            })
    ...
    async get1Entity(Typed, query,projection){
        let conn=await this.collection(Typed)
        try{
    ...
    async findEntity(Typed, query, filter=cursor=>cursor,projection){
        let conn=await this.collection(Typed)
        try{
	...
	nextPage(Typed,{first:count,after},filter=a=>a,projection){
		const [createdAt,_id]=(after||":").split(":")
		return this.findEntity(Typed,{},cursor=>{
	...
	prevPage(Typed,{last:count,before},filter=a=>a,projection){
