...
class DataService{
    ...
    async remove1Entity(cols, query){
        ...
        try{
            ...
            require("./file").resolver(this).Mutation
                .file_clean(null,{host:`${cols}:${_id}`},{app:this})
        ...
        try{
            ...
            removing.forEach(({_id})=>{
                require("./file").resolver.Mutation
                    .file_clean(null,{host:`${cols}:${_id}`},ctx)
            })
    ...
    async get1Entity(cols, query,projection){
        let conn=await this.collection(cols)
        try{
    ...
    async findEntity(cols, query, filter=cursor=>cursor,projection){
        let conn=await this.collection(cols)
        try{
	...
	nextPage(cols,{first:count,after},filter=a=>a,projection){
	...
	prevPage(cols,{last:count,before},filter=a=>a,projection){
