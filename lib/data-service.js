...
const {Db, Server, ObjectId: ObjectID, MongoClient}=require("mongodb")
...
class DataService extends EventEmitter{
	...
	async updateEntity(Type,query,doc){
        ...
        try{
            ...
            await conn.updateMany(query,doc)
    ...
    async removeEntity(Type, query){
        ...
        try{
            ...
            const {deletedCount}=await conn.deleteMany(query)
            return deletedCount
        }catch(e){
			logger.error(e.message)
			return 0
    ...
    async findEntity(Type, query, projection, filter=cursor=>cursor){
        let conn=null, cursor=null
        try{
			conn=await this.collection(Type)
			cursor=await filter(conn.find(query,projection))
	...
	async buildIndexes(indexes){
		...
		const name=spec=>Object.entries(spec).map(([k,v])=>`${k}_${v}`).join("_")
		return Promise.all(Object.keys(indexes).map(async type=>{
			const typeIndexes=indexes[type]
			let conn=null
			try{
				conn=await this.collection(type)
				return await Promise.all(
					typeIndexes.map(arrayfy).map(([{__drop, ...spec},options])=>{
						return __drop ? conn.dropIndex(name(spec)) : conn.createIndex(spec, {name:name(spec), ...options})
					})
				)
			}catch(e){
...
class MongoDataService extends DataService{
	...
	async collection(...names){
		try{
			//const db=await new Db(this.app.apiKey, this.constructor.mongoServer,{w:1}).open()
			const db=this.constructor.mongoServer.db(this.app.apiKey)
