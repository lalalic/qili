 ...
 * @returns 
 */
module.exports=({root, context, blacklist=[], whitelist=[], watches, onChange, recursive=false, console})=>{
    if(!root)
        return 
    ...
    if(watches){
        fs.watch(root,{recursive},(eventType, fileName)=>{
    ...
    const createRequire=(parentModule)=>{
        ...
        function customRequire(modulePath) {
            ...
            if(require.extensions && require.extensions['.js']){
                ...
                if(ext && !(ext in require.extensions) && ext!==".cjs"){
        ...
        customRequire.import=async function(m){
            const resolvedPath=customRequire.resolve(m)
