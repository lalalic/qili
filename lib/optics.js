...
exports.report=function(context){
	...
	function addLatencyToBuckets(buckets, nano){
		buckets.split(".").reduce((currents,a)=>{
			currents.push(a)
			let current=currents.join()
			let time=times[current]||0
			times[current]=time+nano
			return currents
		},[])
	}
	try {
      ...
      queryList.forEach(({ info, resolvers: queryResolvers = [] }) => {
        ...
        queryResolvers.forEach((resolverReport) => {
          ...
          if (resolverReport.endOffset && resolverReport.startOffset) {
            ...
            addLatencyToBuckets(`${query}.${typeName}.${fieldName}`, resolverNanos);
          }
        });
