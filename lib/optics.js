...
exports.report=function(context, options={threshold:100}){
	const {threshold=100}=options
	const at = hrtime => parseInt(((hrtime[0] * 1e9) + hrtime[1])/1e6);
	const path = ({key,prev})=>`${prev ? path(prev)+'/' : ''}${key}`
	...
	try {
		times["/"]=at(context.durationHrTime);
		(context.resolverCalls || []).forEach(({resolverInfo,startOffset,endOffset}) => {
			let key=path(resolverInfo.path);
			let start=at(startOffset)
			let end=at(endOffset)
			if(end-start>=threshold){
				(times[key]=(times[key]||[])).push({at:start,by:end-start})
			}
		});
	} catch (e) {
		logger.error('Optics error', e); 
	}	
  return times
}
