...
module.exports = Super.extend({
		constructor : function (req, res) {
			...
			if(req && req.header && req.params.collection){
				...
				if(this.app._id==Super.config.adminKey && this.kind=='apps')
					Super.noSupport()
			}
		...
		get : function (query, options) {
			...
			if(query._id){
				if(typeof(query._id)=='string' && query._id!=config.adminKey){
					try{
			...
			this.db.open(function (error, db) {
				...
				db.collection(this.kind, function (error, collection) {
					...
					collection[op](query, options||{}, function (error, result) {
						...
						if(op=='findOne'){
							if(query._id && !result){
								console.dir({query,options,kind:this.kind})
								p.reject("Not exists")
							}else
								p.resolve(result)
						}else{
		...
		beforeCreate: function(doc,collection){
			return this.cloudCode().beforeCreate(_.extend({object:doc},this.cloudReq), this.cloudRes)
		},
		afterCreate: function(doc,collection){
			return this.cloudCode().afterCreate(_.extend({object:doc},this.cloudReq), this.cloudRes)
		},
		beforeUpdate: function(doc,collection){
			return this.checkACL(doc,collection,['update'])
				.then(function(old){
					return this.cloudCode().beforeUpdate(_.extend({object:doc,old:old},this.cloudReq), this.cloudRes)
				}.bind(this))
		},
		afterUpdate:function(doc,collection){
			return this.cloudCode().afterUpdate(_.extend({object:doc},this.cloudReq), this.cloudRes)
		},
		beforeRemove: function(doc,collection){
			return this.checkACL(doc,collection,['remove'])
			.then(function(doc){
				return this.cloudCode().beforeRemove(_.extend({object:doc},this.cloudReq), this.cloudRes)
			}.bind(this))
		},
		afterRemove: function(doc,collection){
			return this.cloudCode().afterRemove(_.extend({object:doc},this.cloudReq), this.cloudRes)
		},
			...
			this.db.open(function (error, db) {
				...
				db.collection(this.kind, function (error, collection) {
					...
					this.beforeUpdate(doc,collection).then(function(){
						...
						delete doc._id;
						collection.findAndModify({_id:id}, null, doc, {new:true}, function (error,feedback) {
							if(error) return p.reject(error)
							this.afterUpdate(feedback.value, collection).then(function(){
								p.resolve(changes)
							},_error)
						}.bind(this))
					}.bind(this),_error);
			...
			this.db.open(function (error, db) {
				...
				db.collection(this.kind, function (error, collection) {
					...
					this.beforeRemove(doc,collection).then(function(){
						collection.findAndRemove(doc,function (error,feedback) {
							if(error) return p.reject(error)
							this.afterRemove(feedback.value,collection).then(function(){
								p.resolve(feedback.value)
							},_error)
						}.bind(this))
					}.bind(this),_error)
				}.bind(this))
