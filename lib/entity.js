...
module.exports = Super.extend({
			...
			if(this.app && this.app.logs && this.app.logs.length){
				...
				this.db.open( (error, db) =>{
					...
					db.collection("logs",(error, collection)=>{
						if(error) return p.reject(error);
						collection.insert(this.app.logs,(error)=>{
							if(error)
		...
		checkACL: function(doc, collection, caps){
			...
			collection.findOne({_id:doc._id},(error, doc)=>{
				if(error) return p.reject(error);
				try{
		...
		afterPost:function(doc){
			return {createdAt:doc.createdAt, updatedAt:doc.updatedAt, _id:doc._id}
		},
