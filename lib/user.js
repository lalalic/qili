...
module.exports=Super.extend({
	...
	resolve: function(req, res, next){
		var Self=module.exports,
			token=req.header("X-Session-Token")||req.query['X-Session-Token'],
			user=Self.resolvSessionToken(token);
		if(user){
			req.user=user
			next()
		}else if(req.path=="/"+Self.version+'/'+Self.url+'/login' ||
				req.path=="/"+Self.version+'/'+Self.url+'/requestPasswordReset'){
			next()
		}else {
			next(Self.noSupport())
		}
