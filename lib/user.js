...
function sendSmsCodeTo(phone,template=TEMPLATE_CREATE){
				...
				"post /requestPasswordReset": function(req, res){
					...
					service.checkExists("phone",phone)
						.then(exist=>{
							if(exist){
								service.user=exist
								sendSmsCodeTo(phone,TEMPLATE_RESET)
									.then(code=>service.patch(service.user._id,{
											reset:{
												valid:Date.now(),
												password:encrypt(code)
											}
										}).then(a=>this.send(res,{salt:encrypt(code+"@"+phone)})))
						...
						.then(exist=>{
							if(!!exist==existence)
								sendSmsCodeTo(phone,TEMPLATE_CREATE)
									.then(code=>this.send(res,{salt:encrypt(`${code}@${phone}`)}),this.error(res))
							else
