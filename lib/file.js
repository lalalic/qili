...
exports.typeDefs=`
	...
	extend type Query{
		file_upload_token(host:ID, path:String):FileToken
	}
`
...
exports.resolver={
	URL:{
		parseValue(value) {
			if(value.startsWith(config.qiniu.accessURL)){
				value=value.substr(config.qiniu.accessURL.length+1)
				value=value.split("?")[0]
			}
	...
	Query:{
		file_upload_token(_,{host,path},{app:{app:{apiKey}},user:{sessionToken,_id:author}}){
