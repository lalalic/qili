...
class Main extends Super{
	constructor(req, res){
		...
		this.policy={
			...
			returnBody:'{"url":"'+config.qiniu.accessURL+'/$(key)"}'
		}
		/*
		if(req.query.policy)
			Object.assign(this.policy,JSON.parse(req.query.policy))
		*/

		if(req.query.save){
			this.policy.callbackUrl=config.domain+"1/files";
			this.policy.callbackBody="X-Application-Id="+this.app.apiKey+"&X-Session-Token="+this.user.token
...
Object.assign(Main,{
	...
	routes:{
		"get token":function(req, res){
			res.send({token:new this(req, res).uptoken()})
		},
