...
exports.getStorage=function(appEntity){
	const bucketName=exports.getBucket(appEntity)
	if(isLocalStorageName(bucketName))
		throw new Error("Not correct bucket name")
...
exports.init=function(app){
	const {app:appEntity}=app

	clearTempFileTimers[appEntity.apiKey]=setInterval(()=>{
		app.resolver.Mutation.file_clean({},{filter:{detetable:{$lt:Date.now()}}},{app})
	}, 10*60*1000)
...
function createLocalStorage(bucketName){
	...
	function handler(req, res, next){
		resolve(req, res, function(req, res){
	...
	console.log(`local storage ${bucketName} is ready`)
