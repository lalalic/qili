...
exports.resolver=appEntity=>({
	URL:{
		parseValue(value) {// value from the client
			if(!appEntity.storage || appEntity.storage.length==0)
				return value

			try{
				const {host,pathname}=require("url").parse(value)
				if(appEntity.storage.includes(host)){
		...
		serialize(value) {// value sent to the client
			...
			return `https://${appEntity.storage[appEntity.storage.length-1]}/${value}`
		},
...
exports.getBucket=function(appEntity){
	return appEntity.bucket||(config.debug && config.qiniu.bucket)||appEntity.apiKey.toLowerCase()
}

exports.createStorage=function(appEntity){
	let bucketName=exports.getBucket(appEntity)
	if(bucketName==config.qiniu.bucket){
		return Promise.resolve(config.qiniu.accessURL.split("//").pop())
	}
	...
	return new Promise((resolve,reject)=>rpc.postWithoutForm(requestURI,token, (error, body, respInfo)=>{
		if(error){
			...
			exports.getDomains(appEntity).then(resolve,reject)
		}else{
...
exports.getDomains=function(appEntity){
