...
exports.resolver=appEntity=>({
	...
	Query:{
		file_upload_token(_,{key},ctx){
			const {app:{app:{apiKey},fileProps={}},user:{sessionToken,_id:author}}=ctx
	...
	Mutation:{
		async file_create(_,file,{app,user}){
			...
			let last=await app.get1Entity("File",query)
			if(last){
				file.updatedAt=await app.updateEntity("File",query,file)
				return file
			}else{
		...
		async file_clean(_,{host},{app}){
			...
			let removing=await app.findEntity("File",{host},undefined,{_id:1})
