...
app.use('/:appKey/static', 
	...
	require('./app').resolveApp,
	(req, res, next)=>{
		const uploadDir = req.app.app?.uploadDir
		if (uploadDir && req.headers['content-type']?.includes('multipart/form-data')) {
			multer({dest: uploadDir}).any()(req, res, next);
		}else{
			next();
		}
	},
	...
	(req,res)=>{
		if(req.app && req.app.cloud && req.app.cloud.static){
			if(req.app.app?.uploadDir){
				req.files?.forEach(file=>{
					const nameWithExt=file.path+`.${file.originalname.split('.').pop()}`
					require("fs").rename(file.path, nameWithExt, error=>error)
					req.body[file.fieldname]=nameWithExt
				})
				delete req.files
			}
...
;(()=>{//auto intialize all apps
	...
	async function test(){
		...
		const { cloud: {timeout, __installDir, __moduleBlacklist, __moduleWhitelist, ...apps}, api}=config
		const domain=new URL(api).hostname.split(".").splice(-2).join(".")

		for(const [apiKey, {alias=apiKey}] of Object.entries(apps)){
