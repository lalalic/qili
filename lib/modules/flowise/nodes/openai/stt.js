...
class Node extends ImageNode{
        ...
        tool.func=async function(input, runManager){
            try{
                ...
                const text=await this.openai.audio.transcriptions.create({
                    ...me.clean(inputs), 
                    model:"whisper-1",
                    file:await require('node-fetch')(input)
                })
                return text
            }catch(e){
                return e.message
    ...
    /**it's different from official by minutes */
    static cost(run, app, user){
        const prices=require('../../costs/openai')
        return prices.AudioModels["whisper-1"].Usage*run.output.length*100
    }
}
