...
class HuggingFaceToolNode{
    ...
    async init({inputs:{service,...inputs}},_, options, qili){
        const {url, fn_index, 
            fn_indexes=[fn_index],
            session,
            ...
            async func(input, runManager){
                ...
                try{
                    for(let i=0;i<fn_indexes.length;i++){
                        ...
                        const inputData=await inFormat({input, tool, i, session:hash.session_hash}, runManager)
                        ...
                        result = await outFormat({data, tool, i,input, session:hash.session_hash}, runManager)
                    }
                }catch(e){
        ...
        async outFormat({data,tool}){
            ...
            const urls=await Promise.all(
                images[0].map((image,i)=>{
                    return tool.qili.upload({uri:image, key:`_temp_/1/${now}/${i}`})
                })
            )
            return urls.join(",")
        }
