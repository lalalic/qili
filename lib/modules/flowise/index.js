exports.Flowise=function Flowise({cloudInit, init, finalize, static:staticFx, flowiseRootPath="/", ...props}){
    ...
    return {
        ...
        init(app){
            ...
            flowise=new Flowise()
            flowise.app=staticService.router(flowiseRootPath, `${flowiseRootPath}/index.html`)
            if(!cloudInit){
                require('./magic').init(flowise, app)
            }
            init?.(app, flowise)
            /**
        ...
        finalize(){
            ...
            flowise?.stopApp()
            flowise?.socketServer?.disconnectSockets()
            //flowise?.socketServer?.close();//it will close underlying http server
            finalize?.()
        }
    }
}
