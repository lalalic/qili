exports.Flowise=function Flowise({init, finalize, static:staticFx, initFlowise, flowiseRootPath="/", ...props}){
    ...
    /**
     * it's a hack to pass on parentRunId
     * CallbackManager.configure() will 
     */
    // const {CallbackManager} = require('langchain/callbacks')
    // if(!CallbackManager.supportParentRunIdFromMeta){
    //     CallbackManager.supportParentRunIdFromMeta=true
    //     CallbackManager.prototype.addMetadata=(fx=>function(metadata){
    //         if(metadata.parentRunId){
    //             this._parentRunId=metadata.parentRunId
    //             delete metadata.parentRunId
    //         }
    //         fx.call(this,...arguments)
    //     })(CallbackManager.prototype.addMetadata)
    // }
    ...
    return {
        ...
        static(service){
            ...
            return new Promise(async (resolve)=>{
                ...
                flowise=new Flowise()
                flowise.app=service.router(flowiseRootPath)
                await initFlowise?.({flowise, qili:app, Static:service})
                 resolve()
            })
        },
        init(app){
