...
module.exports={
    ...
    credentials:{
        ...
        aliApi(run){
            const {node, model=node?.inputs?.modelName, prompt_tokens=0, completion_tokens=0, image_tokens=0}=run
            return Math.max(100,(ali[model]||ali["qwen-turbo"])*(prompt_tokens+completion_tokens+output_tokens+input_tokens+image_tokens))
        },
        baiduApi(run){
            return 100
        },
