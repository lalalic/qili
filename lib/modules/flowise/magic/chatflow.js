...
module.exports=class ChatFlow{
    ...
    async predict(question, metadata, config){       
        ...
        return new Promise((resolve, reject)=>{
            ...
            const prediction=this.flowise[require('./extend-flowise').Flowise_Prediction_Function]
            ...
            try{
                ...
                require('./extend-flowise').injectRunMetadata(currentConfig,{
                    ...
                    predict:async(chatflowId, nodeId, config, parentRunId)=>{
                        const subChatflowConfig={overrideConfig:{}, ...currentConfig, ...config}
                        subChatflowConfig.overrideConfig.overrideEndingNodeAnchorsGraph=require('./extend-flowise').extractNodeGraph(nodeId, this.graph)
                        const chatflow=new ChatFlow({...this.props, parentRunId, id:chatflowId, config:subChatflowConfig})
    ...
    async overrideFlowData(chatflow){
        ...
        const ctx=this.ctx
        //all nodes has user information
        graph.nodes.forEach(node=>node.data.ctx=ctx)
        ...
        for (let i=0;i<files.length;i++){
            ...
            if(document?.data.inputs.embeddings){
                const urls=await Promise.all(
                    anyFile.data.inputs.file.split(",").filter(a=>!!a).map(async url=>{
                        const can=await this.qili.resolver.Document.canEmbed({},{url},{app:this.qili, user})
                        if(can){
                            return url
                        }})
                )
                ...
                if(document.data.name=="qiliUpsert"){
                    if(document.data.inputs.indexName){
                        document.data.inputs.indexName=`User:${chatflow.author}:${document.data.inputs.indexName}`
                    }else{
                        document.data.inputs.indexName=`User:${chatflow.author}:ChatFlow:${chatflow.id}:${document.data.id}`
                    }
                }
            }
        }
