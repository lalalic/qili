module.exports=class ChatFlow{
    ...
    async predict(question, metadata, config){       
        ...
        return new Promise((resolve, reject)=>{
            ...
            try{
                ...
                require('../utils/extractRunMetadata').injectRunMetadata(currentConfig,{
                    ...this.ctx,
                    monitor:this.monitor,
                    silent:true,
                    //predict sub chatflows
                    predict:async(chatflowId, nodeId, config, parentRunId)=>{
                        const monitor=this.monitor.clone({chatflowId, rootRunId:parentRunId})
                        ...
                        monitor.start('sub', runId)
                        const result=await chatflow.predict()
                        monitor.end('sub', runId)
                        return result
                    },
                    ...metadata
                })
    ...
    clone(props){
        return new ChatFlow({...this.props, ...props})
    }
    ...
    async evaluate(){
        ...
        for(const a of evaluatorNodes){
            const {enabled=true, testCaseName:name="", input, history, overrideConfig, shouldEvaluate=true}=a.data.inputs
            if(this.config.overrideConfig?.enabled?.[a.id]===false || 
                (this.config.overrideConfig?.enabled===undefined && !enabled)){
            ...
            */
            ...
            try{
                result=await this.clone({
                    config:{
                        question:input,
                        overrideConfig: eval(`(${overrideConfig||'{}'})`),
                        history: eval(`(${history||'[]'})`)
                    }
                }).predict(
                    input, 
                    {evaluating:a.id, shouldEvaluate}, 
                )
        ...
        const validResults=results.filter(a=>!!a.value)
        return {
            value: validResults.find(a=>a.value=='N' || a.value.indexOf(' N')!=-1) ? 'N' : 'Y',
            score: Math.ceil(100*validResults.reduce((score, a)=>score+a.score,0)/validResults.length)/100,
            results,
        }
    }
    ...
    async overrideFlowData(chatflow){
        ...
        if(this.config.overrideConfig){
            ...
            //flowise way override
            const {replaceInputsWithConfig}=require("./graph")
            graph.nodes.forEach(node=>replaceInputsWithConfig(node.data, this.config.overrideConfig))
        }
         ...
         * 
         */
        const files=graph.nodes.filter(a=>a.data.name=="anyFile" && a.data.inputs.file.trim())
        for (let i=0;i<files.length;i++){
            const anyFile=files[i]
            const vectorStore=targetOf(anyFile, graph)
            if(vectorStore?.data.inputs.embeddings){
                if(vectorStore.data.name=="qiliUpsert"){
                    // if(this.config.overrideConfig?.indexName){
                    //     vectorStore.data.inputs.indexName=this.config.overrideConfig.indexName
                    ...
                    // }
                    /**
                     * this.author is payer, whose document is 
                     */

                    if(!vectorStore.data.inputs.indexName){
                        vectorStore.data.inputs.indexName=`ChatFlow/${chatflow.id}/${vectorStore.id}`
                    }

                    const ctx=this.ctx
                    vectorStore.data.inputs.indexName=vectorStore.data.inputs.indexName.split("/").map(a=>{
                        return a.replace(/{(.*?)}/g, function(matched, name){
                            return name in ctx ? ctx[name] : name
                        })
                    }).join("/")

                    const {indexName}=vectorStore.data.inputs
                    const {file}=anyFile.data.inputs
                    if(file){
                        const urls=await this.qili.resolver.Document.patch({},{indexName, file}, {app:this.app, user})
                        anyFile.data.inputs.file=urls.filter(a=>!!a).join("\n").trim()
                    }
                }
            }
        }
