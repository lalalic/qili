...
function monitorFactory(flowise, qili) {
    ...
    return class QiliMonitor extends flowise.Handlers.LLMonitor {
        ...
        async handleChatModelStart(llm, baseMessages, _runId, _parentRunId, extraParams){
            if(this.supportImageContent(llm, extraParams)){
                baseMessages.forEach(messages=>{
                    messages.forEach(message=>{
                        if(message._getType()=="human"){
                            message.content=this.parseMarkdown(message.content)
                        }
                    })
                })
            }
        ...
        supportImageContent(llm){
            return flowise.VisionModels?.indexOf(llm.kwargs?.model)!=-1
        }

        parseMarkdown(content){
            const info=[...content.matchAll(/\!\[(?<title>.*?)\]\((?<url>.*?)\)/g), null].reduce((data, matched)=>{
                if(!matched){//last
                    data.content.push({type:"text", text: content.substring(data.i)})
                    return data
                }
                const [whole]=matched
                const {index, groups:{title, url}}=matched
                data.content.push({type:"text", text: content.substring(data.i, index)})
                data.content.push({type:"image_url", image_url: {url, title}})
                data.i=index+whole.length
                return data
            },{content:[], i:0})

            return info.content.length>1 ? info.content : content
        }
