...
function monitorFactory(flowise, qili) {
    ...
    return class QiliMonitor extends flowise.Handlers.LLMonitor {
                ...
                async trackEvent(type, event, data) {
                    ...
                    switch (event) {
                        ...
                        case "end": {
                            ...
                            me.push(() => qili.patchEntity(...args).then(a => me.track(...args, cost)));
        ...
        /** lazy to pass */
        track(_, filter, docOrPatch, possibleToolCost) {
            ...
            if (status == "success") {//end
                    const cost=qili.resolver.Run.cost(
                        {},
                        { runId: _id, rootRunId: this.rootRunId, chatflowId: this.chatflowId, nodeAmount:this.nodeIndex, possibleToolCost}, 
