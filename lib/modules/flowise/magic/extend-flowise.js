        ...
        monkeyIntercept(a.constructor.prototype, 'run', runChainOrAgentNode => async function (node, input, options) {
            ...
            monkeyIntercept(chainOrAgent,'_call', _callChainOrAgent => async function (values, runManager) {
                ...
                if(preAction){
                    ...
                    input=extractText(await preAction.call({...inputs, input}, config))
                }
    ...
    allNodes.filter(a => a.category == "Tools" && a.type!="Adaptable").forEach(a => {
        ...
        monkeyIntercept(a.constructor.prototype,'init', init => async function (node) {
            ...
            monkeyIntercept(tool, '_call', fx2 => async function (input, runManager) {
                ...
                let output=await fx2.call(this, input, runManager)
                if(isCustomTool){
                    /*
                    * agent tool output will convert to FunctionMessage when back to agent
                    * if output is not string, output must be {content, name:[function name]}
                    */
                    if(typeof(output)=="object"){
                        output={...output}//output is a Proxy if it's object
                        if(!tool.returnDirect){
                            const {text:$2=$1, content:$3=$2, ...others}=output
                            return {content: $3, output:$3, ...others, name: tool.name}
                        }
                    }
                }
                return output
