...
function extendCosts({nodes, credentials},flowise){
    const {componentCredentials, componentNodes}=flowise.nodesPool
    ...
    return class QiliMonitor extends flowise.Handlers.LLMonitor{
        ...
        track(_, filter, docOrPatch){
            ...
            if(status=="success" || status=="error"){
                (async()=>{
                    const cost=await qili.resolver.Run.cost({...data}, {runId:_id, rootRunId: this.rootRunId, chatflowId:this.chatflowId}, {app:qili, user:{_id:author}})
                    if(cost){
