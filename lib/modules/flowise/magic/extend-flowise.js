...
exports.monitorFactory=function monitorFactory(flowise, qili){
    function safe(node){
        node.inputs=JSON.parse(JSON.stringify(node.inputs,(key,value)=>{
            if(key.indexOf(".")!=-1 || ["undefined", "function"].indexOf(typeof(value))!=-1){
                return
            }
            return value
        }))
        ...
        monkeyIntercept(a.constructor.prototype, 'run', f1 => async function (node, input, options) {
            ...
            monkeyIntercept(chainOrAgent,fxName, f2 => async function (input, callbacks) {
                ...
                if(preAction){
                    ...
                    input=await preAction.call({...inputs, input}, callbacks)
                }
