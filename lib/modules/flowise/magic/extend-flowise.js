    ...
    allNodes.filter(a => !!a.run).forEach(a => {
        ...
        monkeyIntercept(a.constructor.prototype, 'run', f1 => async function (node, input, options) {
            ...
            if(false && chainOrAgent.call){
                monkeyIntercept(chainOrAgent,'call', f2 => async function (input, config) {
                    ...
                    if(functionCall){//agent
                        ...
                        if(tool){
                            ...
                            try{
                                prediction={output:await tool.call(input, config)}
                            }catch(e){
                    ...
                    if(!prediction){
                        prediction= await f2.call(this, input, config)
                    }
                ...
                return await f1.call(this, ...arguments, qili, flowise)
            }else{
                const runMetadata=exports.extractRunMetadata(options)
                ...
                if(preAction){
                    const inputs=strip(preAction.schema,preAction.metadata.node.inputs)
                    const transformed=await preAction.call({...inputs, input}, config)
                    input=extractText(transformed)
                }

                let prediction=null
                if(functionCall){//agent
                    const functionCallKey=functionCall.toLowerCase()
                    const toolsByName = Object.fromEntries(chainOrAgent.tools?.map((t) => [t.name.toLowerCase(), t]));
                    let tool=toolsByName[functionCallKey]
                    if(!tool){
                        tool=Object.values(toolsByName).find(a=>functionCallKey==a.metadata?.node?.inputs?.uname?.toLowerCase())
                    }
                    if(tool){
                        ...
                        try{
                            prediction=extractText(await tool.call(input, config))
                        }catch(e){
                            console.error(e)
                        }
                    }
                ...
                if(!prediction){
                    prediction= await f1.call(this, node, input, options, qili, flowise)
                }

                if(postAction){
                    const inputs=strip(postAction.schema,postAction.metadata.node.inputs)
                    prediction=extractText(await postAction.call({...inputs, input:extractText(prediction)}, config))
                }

                if(runMetadata.evaluating && evaluators?.length>0 && !runMetadata.parentRunId){
                    const evaluator=evaluators.find(a=>a.metadata.node.id==runMetadata.evaluating)
                    if(runMetadata.shouldEvaluate){
                        const evaluateResult= await evaluator.evaluate({
                            input,
                        ...
                        evaluateResult.parts={input,prediction}//without output/text, so chatflow.predict return whole result
                        return evaluateResult
                    }else{
                        return {
                            shouldEvaluate:false,
                            parts:{input, prediction}
                        }
                    }
                }
                return prediction
                    ...
                    */
                    if(typeof(output)=="object"){
                        ...
                        if(!tool.returnDirect){
                            ...
                            return {content: $3, text:$3, ...others, name: tool.name}
                        }
                    }
                }
                return output
