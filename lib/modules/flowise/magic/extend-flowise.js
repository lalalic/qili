const { Flowise_Prediction_Function,}=require('./constants')
const monkeyIntercept =require("../utils/monkey-intercept")
const extractText = require("../utils/extract-text")
const { extractRunMetadata } = require('../utils/extractRunMetadata')
...
exports.extendFlowise=async function initFlowise({flowise, qili}){
    flowise.extractRunMetadata=extractRunMetadata
...
function extendEmbeddings(flowise){
    Object.values(flowise.nodesPool.componentNodes)
        ...
        .forEach(vectorStoreNode=>{
            monkeyIntercept(vectorStoreNode.constructor.prototype, 'init', fx=>async function(node, question, options){
                const retrieverOrStore=await fx.call(this, ...arguments)
                const vectorStore=store=retrieverOrStore.vectorStore||retrieverOrStore
                const ctx=extractRunMetadata(options)
                const monitor=ctx.monitor.monitor
                const uuid=require('uuid').v4
                
                ;['similaritySearch','similaritySearchWithScore'].forEach(fxName=>{
                    vectorStore[fxName]= vectorStore[fxName]
                    monkeyIntercept(vectorStore, fxName, fx=>async function(query){
                        const runId=uuid()
                        monitor.trackEvent('VectorSearch','start',{
                            runId,
                            parentRunId: ctx.monitor.rootRunId,
                            extra:{...retrieverOrStore.metadata},
                            input:query
                        })
                        const docs= await fx.call(this, ...arguments)
                        monitor.trackEvent('VectorSearch', 'end', {
                            runId,
                            output:`${docs.length} docs`
                        })
                        return docs
                    }) 
                })
...
function extendRunnable(allNodes, qili, flowise) {
    allNodes.filter(a => !!a.run).forEach(a => {
        ...
        if(a.category=="Agents" && !!a.inputs.find(a=>a.name=="tools")){
            a.inputs.push({
                label:"Function Call Directly",
                name: "functionCall",
                type:"string",
                optional:true,
        ...
        monkeyIntercept(a.constructor.prototype, 'run', runChainOrAgentNode => async function (node, input, options) {
            /**
            ...
            const { instance: chainOrAgent, category, inputs:{evaluators, postAction, preAction, functionCall} }=node
            const runMetadata=extractRunMetadata(options)
            const callbacks=[runMetadata.monitor]
            if(!chainOrAgent.call)
                return await runChainOrAgentNode.call(this, ...arguments, qili, flowise)
 
            if(chainOrAgent.call){
                //inject monitor at the entry node
                monkeyIntercept(chainOrAgent,'call', callChainOrAgent => async function (input, config){
                    return await callChainOrAgent.call(this, input, Array.isArray(config) ? callbacks : config)
                },'inject monitor')
            }

            //
            monkeyIntercept(chainOrAgent,'_call', _callChainOrAgent => async function (values, runManager) {
                ...
                if(preAction){
                    ...
                    input=extractText(await preAction.invoke({...inputs, input}, config))
                }

                let prediction=null
                if(functionCall){//agent
                    const functionCallKey=functionCall.toLowerCase()
                    const toolsByName = Object.fromEntries(this.tools.map((t) => [t.name.toLowerCase(), t]));
                    let tool=toolsByName[functionCallKey]
                    if(!tool){
                        tool=Object.values(toolsByName).find(a=>functionCallKey==a.metadata?.node?.inputs?.uname?.toLowerCase())
                    }
                    if(tool){
                        if(tool.metadata?.node.name=="customTool"){
                            try{
                                const a=JSON.parse(input.input)
                                if(typeof(a)=="object"){
                                    input=a
                                }
                            }catch(e){
                        ...
                        try{
                            prediction=await tool.call(input, config)
                            if(typeof(prediction)=="string"){
                                prediction={output:prediction}
                            }else if(!('output' in prediction)){//agent or chain return prediction.output
                                prediction.output=extractText(prediction)
                            }
                        }catch(e){
                ...
                if(!prediction){
                    prediction= await _callChainOrAgent.call(this, {...values, input}, runManager)
                }

                const outputKey=extractText.extractOutputKey(prediction)
            
                if(postAction){
                    const inputs=strip(postAction.schema,postAction.metadata.node.inputs)
                    const output=await postAction.invoke({...inputs, input:extractText(prediction)}, config)
                    prediction[outputKey]=extractText(output)
                }

                if(evaluators && runMetadata.evaluating && !runMetadata.parentRunId){
                    ...
                    if(runMetadata.shouldEvaluate){
                        ...
                        evaluateResult.parts={input:evaluateInput, output:outputText}
                        return {[outputKey]:JSON.stringify(evaluateResult)}
                    }else{
                        return {[outputKey]:JSON.stringify({
                            shouldEvaluate:false,
                            parts:{input:values.input, output:outputText}
                        })}
                    }
                }
                return prediction
...
function extendAllToolsWithPreTools(allNodes) {
    allNodes.filter(a => a.category == "Tools" && a.type!="Adaptable").forEach(a => {
        ...
        monkeyIntercept(a.constructor.prototype,'init', init => async function (node) {
            ...
            const isCustomTool=node.name=="customTool"
            if(isCustomTool){
            ...
            monkeyIntercept(tool, '_call', fx2 => async function (input, runManager) {
                ...
                const output=await fx2.call(this, input, runManager)
                if(runManager.metadata.cost){
                    if(!runManager.tags)
                        runManager.tags=[]
                    runManager.tags.push(`cost:${Math.ceil(runManager.metadata.cost)}`)
                    delete runManager.metadata.cost
                }
                return output
            })
            return tool
...
function makeAllNodesMetadataReady(allNodes, qili, flowise) {
    allNodes.forEach(a => {
        a.inputs=a.inputs||[]
        a.inputs.unshift({
            label:"Node Reference Name",
            name:"uname",
            type:"string",
            description:"Node id replacement to override node settings",
            additionalParams:true,
            optional:true
        })
...
exports.overrideGraphInputs=function(patch, graph){
    ...
    Object.keys(patch).forEach(nodeId=>{
        const node=nodes.find(a=>a.id==nodeId || a.data.inputs?.uname==nodeId)
        if(node){
            if(patch[nodeId]){
