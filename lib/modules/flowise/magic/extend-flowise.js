exports.Flowise_Prediction_Function="processPrediction"
exports.Qili_Monitor_Name='qili_monitor'
...
exports.extendFlowise=async function initFlowise({flowise, qili}){
    dontCloneDuringPrediction(exports.Flowise_Prediction_Function,flowise)
    ...
    extendAdditionalHandler(flowise)
...
function extendAdditionalHandler(flowise){
    monkeyIntercept(flowise.Handlers, 'additionalCallbacks', fx=>function(nodeData, options){
        const callbacks=fx.call(this, ...arguments)
        callbacks.unshift(new flowise.Monitor(options.runMetadata))
        return callbacks
    })
}
...
exports.monitorFactory=function monitorFactory(flowise, qili){
    ...
    return class QiliMonitor extends flowise.Handlers.LLMonitor{
        constructor({author:userId, chatflowId, runMonitorSocket, rootRunId}){
            ...
            this.name=exports.Qili_Monitor_Name
...
function extendRunnable(allNodes, qili, flowise) {
    allNodes.filter(a => !!a.run).forEach(a => {
        ...
        monkeyIntercept(a.constructor.prototype, 'run', f1 => async function (node, input, options) {
            ...
            return await f1.call(this, node, input, {...options, runMetadata})
        },'runnable')
    })
}
...
function makeAllNodesMetadataReady(allNodes, qili, flowise) {
    allNodes.forEach(a => {
        monkeyIntercept(a.constructor.prototype, 'init' ,fx => async function ({id,name, credential, inputs, ctx}, input, options) {
            const langchainNode = await fx.call(this, arguments[0], input, {...options, runMetadata:ctx}, qili, flowise, ctx)
