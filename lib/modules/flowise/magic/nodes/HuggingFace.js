...
module.exports=function extend(node){
    ...
    Object.assign(HuggingFaceToolNode.services,{
        ...
        Text2Image:{
            ...
            async outFormat({data,tool,input}, runManager){
                const images=data[0], now=uuid.v4()

                const urls=await Promise.all(
                    images.slice(0,1).map((image,i)=>{
                        return tool.qili.upload({uri:image, key:`_temp_/1/${now}/${i}`}, runManager?.metadata?.author)
                    })
                )
                return {output:urls.map(src=>`![image](${src})`).join("")}
            }
        },
        Image2Tag:{
            name:"Image2Tag",
        ...
        Image2Chat:{
            name:"Image2Chat",
            label:"Image2Chat",
            description:"good for image description, details, good for chat about an image. the input must be a url.",
