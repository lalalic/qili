...
exports.graphql=function(app,user){
	...
	const extractLog=({ document, variables, operationName, result })=>{
		...
		return {
			...
			variables:app.logVariables(operationName, variables),
...
class Application extends DataService{
	...
	logVariables(variables,operationName){
		if(!variables)
			return variables
		try{
			return typeof(this.cloud.logVariables)=="function" ? this.cloud.logVariables({...variables},operationName) : undefined
		}catch(e){
