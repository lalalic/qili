...
class Application{
    ...
    requestToken(contact){
		let code="1234"//`${Math.floor(Math.random()*900000) + 100000}`
        return this.getUserByContact(contact)
            .then(user=>{
                let uid=user&&user._id||contact
                return this[`send${isEmail(contact) ? "Email" : "Phone"}Token`](contact, uid,code)
                    .then(()=>({user,code,uid}))
            })
            .then(({user,code,uid})=>new Promise((resolve,reject)=>{
                this.passwordless.storeOrUpdate(code,uid,60*1000,null, e=>{
                    if(e){
                        reject(e)
                    }else{
                        resolve(!!user)    
                    ...
                    this.passwordless.authenticate(token,uid, (e,valid)=>{
                        if(valid){
							...
							if(user){
								...
								user={...makeContact(contact),createdAt: new Date()}
    ...
    get schema(){
        return makeExecutableSchema({
            typeDefs:[
				...
				require("./role").typeDefs,
				require("./file").typeDefs,
            ],
            resolvers:merge(
