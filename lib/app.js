...
class Application extends DataService{
	...
	compileCloudCode(){
		...
		try{
			...
			const {cloudCode, staticRoot, vmRequire, localCloudCodePath, requireRoot}=(code=>{
				const {unsupportedModules=[]}=config.cloud
				let {code:localCloudCodePath,root:staticRoot, codeExts=["js","jsx","json","ts"]}=config.cloud[this.app.apiKey]||{}
				let vmRequire=(a)=>{
					return
				}
				let requireRoot=null
				if(localCloudCodePath){
					if(fs.statSync(localCloudCodePath).isDirectory()){
						...
						const myRequire=module.createRequire(require("path").resolve(localCloudCodePath, pkg.main||"./index.js"))
					...
					try{
						code=fs.readFileSync(localCloudCodePath,{encoding:"utf8"})
				...
				return {cloudCode:code, staticRoot, vmRequire, localCloudCodePath, requireRoot}
			})(this.app.cloudCode);
			
			const globalContext={
				Cloud,setTimeout, setInterval, clearTimeout, clearInterval, 
				console:this.logger,
				process:{
			...
			const vm=new NodeVM({
				...
				require:{
					root:requireRoot,
					external:true,
					//builtin: module.builtinModules.filter(a=>unsupportedModules.indexOf(a)==-1)
				}
			})
			vm.run(cloudCode||"")
