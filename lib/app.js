...
const APPS=exports.Cache={
	...
	remove(_id, reason){
		if(this[_id]){
			...
			this[_id].logger.info(`cleared from cache`)
			delete this[_id]
			Application
				.getDataLoader("App")
				.clear(apiKey)
...
class Application extends DataService{
    ...
    static create(apiKey){
		...
		if(config.cloud[apiKey]){
			...
			return ModuleFile.getStorage(entityApp)
				.catch(error=>ModuleFile.createStorage(entityApp))
				.then(domain=>{
    ...
    constructor(app){
		super(app)
		this.logger=require("log4js").getLogger(app.apiKey)
		this.logger.addContext('appKey',app.apiKey)
	...
	compileCloudCode(){
		...
		const {getStorage:_,createStorage:_1,destroyStorage:_2,getBucket:_3, resolver:fileResolverFx, ...FileModule}=ModuleFile
		...
		try{
				...
				if(localCloudCodePath){
					if(fs.statSync(localCloudCodePath).isDirectory()){
						...
						if(!isDev){
							...
							this.logger.info(`installing dependencies for cloud code with ${cmd} ${args.join(" ")}`)
							const {status, stderr}=require("child_process")
								.spawnSync(
									cmd, 
									args, 
									{cwd:localCloudCodePath, stdio:['ignore','ignore','inherit'],}
								)
							if(status!=0){
								throw new Error(`packages install failed with exit code ${status}\n${stderr.toString()}`)
							}
						}
			...
			const context=require("vm").createContext({
				...
				WebSocket, TextDecoder, TextEncoder,Buffer, URL, File, Blob, Response, Request, FormData,
			...
			;(async ()=>{// make sure init in order
				this.logger.info(`starting initializing`)
				for(const {init} of modules){
					if(init){
						await safe(()=>init(this))
					}
				}
				this.logger.info(`started with ${modules.length} modules: ${modules.map(a=>a.name||"~anonymous").filter(a=>!!a).join(",")}`)
			})();
			...
			this.logger.error(`[${this.app.apiKey}] compile error: ${e.message}`)
