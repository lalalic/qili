...
class Application extends DataService{
	...
	runQL(query,variables,root,ctx,operationName){
		...
		if(operationName){
			...
			operationName=query.substring(0,end).split(/\s+/).filter(a=>!!a).pop()
		}
		let document=null
		let {schema,context,extensions,formatError=require("graphql").formatError}=exports.graphql(this,ctx.user)
		
		return Promise.resolve(function(){
			try{
				document=parse(query)
				return execute(schema,document,root,{...ctx,...context},variables,operationName)
			}catch(e){
				return {errors:[e]}
		}}())
			.then(result=>{
			...
			.then(result=>{
				if (result && result.errors)
					result.errors = result.errors.map(formatErrors)
				return result
			})
	}
