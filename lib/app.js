...
const APPS=exports.Cache={
	remove(_id){
		let key=Object.keys(this).find(k=>this[k] && this[k].app && this[k].app._id==_id)
		if(key){
			delete this[key]
		}
	}
...
exports.resolve=persitedQuery=>(req, res, next)=>{
    Application.create(req.headers['x-application-id'])
        .then(app=>{
            req.app=app
			if(persitedQuery){
				req.body.query=app.getPersistedQuery(req.body.id)||req.body.query
			}
            next()
        })
        .catch(e=>{
...
class Application{
    ...
    static create(id){
        ...
        return new Application({apiKey:config.adminKey})
            .get1Entity("apps",{apiKey:id})
            .then(app=>{
    ...
    constructor(app){
        ...
        this.app=app
		this.passwordless=new PasswordlessStore(`mongodb://${config.db.host}:${config.db.port}/${app.apiKey}}`)
	}
	...
	cloudCode(){
		...
		try{
			...
			`
			this.app.resolver=hasComment ? merge(Cloud.resolver, Comment.resolver) : Cloud.resolver
			
			this.app.persistedQuery=Cloud.persistedQuery
		}catch(e){
            console.error(e)
		}
	}
	...
	getPersistedQuery(id){
		if(!this.app.resolver)
			this.cloudCode()
		return this.app.persistedQuery ? this.app.persistedQuery[id] : null
	}
    ...
    get schema(){
        ...
        try{
            ...
            return this.app.schema=makeExecutableSchema({
                typeDefs:[
                    Schema.typeDefs,
    				User.typeDefs,
    				Role.typeDefs,
