	...
	compileCloudCode(){
		...
		try{
			...
			const context=require("vm").createContext({
				...
				...(eval(`(${process.env[`${this.app.apiKey}.jsContext`]||'{}'})`)),
			...
			context.require=require('./vm-require')({
				...
				blacklist:[
					...
					...((process.env[`${this.app.apiKey}.moduleBlacklist`]||'').split(",").map(a=>a.trim()))
				],
				whitelist:[
					...config.cloud.__moduleWhitelist, 
					...((process.env[`${this.app.apiKey}.moduleWhitelist`]||'').split(",").map(a=>a.trim()))
				],
