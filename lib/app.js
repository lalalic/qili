		...
		try{
			...
			const {cloudCode, staticRoot}=(code=>{
				...
				if(localCloudCodePath){
					try{
						logger.info(`cloud code[${this.app.name}] from ${localCloudCodePath}`)
						code=require("fs").readFileSync(localCloudCodePath)
					}catch(e){
						logger.error(e)
					}
				}
				if(staticRoot){
					logger.info(`Content not from ${this.app.name} is served from ${staticRoot}`)
				}
			...
			modules.reduce((a,module)=>{
				if(module.init){
					module.init(cloud)
					logger.debug(`module ${module.name} inited`)
				}
			}, cloud)
			logger.debug(`App[${this.app.name}] cloud code compiled with modules:${modules.map(a=>a.name).filter(a=>!!a).join(",")}`)
