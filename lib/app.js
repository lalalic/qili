...
function finalize(){
	Object.values(APPS).filter(a=>'finalize' in a).forEach(a=>a.finalize())
}
...
exports.resolveQuery=(req, res, next=a=>a)=>{
	...
	const resolve=()=>{
		...
		req.body.query= req.body.query||query|| (req.app.isDev && req.query.query)
		next()
	}	
...
class Application extends DataService{
		...
		if(config.cloud[apiKey]){
			...
			return File.getStorage(entityApp)
				...
				.then(domain=>{
					...
					return new Application(entityApp)
				})
				.then(app=>{
	...
	compileCloudCode(){
		...
		try{
			...
			const {cloudCode, staticRoot, vmRequire, localCloudCodePath}=(code=>{
				const {unsupportedModules=[]}=config.cloud
				let {code:localCloudCodePath,root:staticRoot, codeExts=["js","jsx","json","ts"]}=config.cloud[this.app.name]||{}
				let vmRequire=(a)=>{
					return
				}

				if(localCloudCodePath.match(/^https?\:\/\//)){
					const name=localCloudCodePath.split("?")[0].split("/").pop().split(".")[0]
					const {error}=require("child_process")
						.spawnSync(
							"git", 
							["clone","--depth", "1", localCloudCodePath], 
							{cwd:config.cloud.__installDir, stdio:['ignore','ignore','inherit']}
						)
					this.logger.info(`git clone ${localCloudCodePath}`)
					error && this.logger.error(error)
					localCloudCodePath=require('path').resolve(config.cloud.__installDir, name)
				}
				...
				if(localCloudCodePath){
					if(fs.statSync(localCloudCodePath).isDirectory()){
						...
						const args=["install", "--ignore-optional", "--ignore-scripts"]
						const {error}=require("child_process")
							.spawnSync(
								cmd, 
								args, 
						...
						const pathRequire=require('require-from-path')(localCloudCodePath)
						vmRequire=function(name){
							name=name.split("!").pop()//unwrap inline webpack loader
							if(unsupportedModules.indexOf(name)!=-1){
								return new Error("unsupported module")
							}
							const ext=name.match(/\.(?<ext>[^.]+)$/)?.groups?.ext
							if(ext && codeExts.indexOf(ext)==-1){
								return fs.readFileSync(pathRequire.resolve(name))
							} 

							return pathRequire(name)
						}
						vmRequire.resolve=function(name){
							name=name.split("!").pop()//unwrap inline webpack loader
							return pathRequire.resolve(name)
						}
					...
					try{
						...
						fs.watchFile(localCloudCodePath,(current, prev)=>{
							this.logger.info(`cloud code changed, will reload it`)
				...
				return {cloudCode:code, staticRoot, vmRequire, localCloudCodePath}
			})(this.app.cloudCode);
			...
			const resolver=merge({},...modules.map(a=>a.resolver).filter(a=>!!a).reverse())
			...
			;(()=>{//resolvePubsub
				...
				if(cloud.pubsub){
					cloud.pubsub.onConnect=function(){
						onConnectFns.forEach(fn=>fn(...arguments))
					}
					cloud.pubsub.onDisconnect=function(){
						onDisconnectFns.forEach(fn=>fn(...arguments))
					}
				}
			})();
