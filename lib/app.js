...
class Application extends DataService{
	...
	createSocketServer(opt={path:"/socket.io"}){
		const {Server}=require('socket.io')
		opt={path:"/socket.io",...opt}
		opt.path=`/${config.version}/${this.app.apiKey}${opt.path}`
		const io=new Server(config.httpServer,opt)
		return io
	}
	...
	async upload({uri, key, ...variables}, user={}){
		...
		const {token, key:mayRevisedKey}=await this.resolver.Query.file_upload_token({},{...variables, key},{app:this,user})

		if(this.app.localStorageHandler){
			const url= this.app.localStorageHandler.pipe({stream, key, app:this})
			return Promise.resolve(`https://${this.app.apiKey}.${this.app.vhost}/${url}`)
		}
