...
const APPS=exports.Cache={
	add(one){
		...
		one.clearTempFileTimer=setInterval(()=>one.clearTempFile(), 10*60*1000)
	},
	remove(_id){
		if(this[_id]){
			const {app:{apiKey,name}, clearTempFileTimer}=this[_id]
			clearTempFileTimer && clearInterval(clearTempFileTimer)
			delete this[_id]
			Application
				.getDataLoader("App")
				.clear(apiKey)
...
class Application extends DataService{
	...
	cloudCode(){
		...
		try{
			...
			const {cloudCode, staticRoot}=(code=>{
				const {code:localCloudCodePath,root:staticRoot}=config.cloud[this.app.name]||{}
			...
			require("vm").runInNewContext(cloudCode||"",
				{
					...
					require(a){
						return {
							...
							"graphql-redis-subscriptions":require("graphql-redis-subscriptions"),
			...
			const resolvers=merge({},...modules.map(a=>a.resolver).filter(a=>!!a).reverse())
			...
			console.debug(this.cloud.persistedQuery)
			*/
		}
		return this.cloud
	}
	...
	async upload({uri, key, ...variables}){
		const {data:{token:{token, key:mayRevisedKey}}}=await this.runQL(
			'file_token_Query',
			{...variables,key}
		)
		...
		return new Promise((resolve, reject)=>{
			qiniu.io.putReadable(token, mayRevisedKey, stream, null, (error, result, rs)=>{
				if(error){
					reject(error)
				}else{
	...
	clearTempFile(){
		this.cloud.resolvers.Mutation.file_clean({},{filter:{detetable:{$lt:Date.now()}}},{app:this})
	}
}
