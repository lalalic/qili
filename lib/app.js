...
Object.assign(module.exports=Application,{
	...
	routes:Object.assign({},Entity.routes,{
		"post": function(req, res, next){
			if(req.body && req.body._id){
				...
				Entity.routes.post.apply(this,arguments)
			}
...
class AppMan extends Entity{
		...
		return this.targetAppPromise.then(()=>{
			...
			this.db.open(function(error, db){
				...
				db.listCollections().toArray((error, collections)=>{
					...
					let indexes={}
					Promise.all(collections.map(kind=>new Promise((resolve,reject)=>{
						indexes[kind.name]=[]
						db.collection(kind.name,function(error,collection){
							collection.indexInformation({full:true},function(error, infos){
								if(error){
									reject(error)
								}else{
									infos.forEach(info=>{
					...
					}))).then(()=>p.resolve(indexes),p.reject)
				})
			})
	...
	getCollectionData(query,options){
		return this.targetAppPromise.then(()=>this.get(query,options))
	}
}
...
Object.assign(AppMan, {
	...
	routes:{
		...
		"get /schemas/:collection": function(req, res){
			var service=new this(req, res)
			var [query,option]=this.parseQuery(req.params.id,req.query);
			service.getCollectionData(query,option)
