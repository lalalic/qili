...
exports.create=(root)=>{
	...
	return Object.create({
		...
		auth2(options){
			Object.entries(options).forEach(([name, {strategy, scope, redirectUrl, domain=""}={}])=>{
				if(!strategy)
					return 
				...
				this.on(`/auth/${name}`,Object.assign((req, res)=>{
					const authenticate1=conf.passport.authenticate(key, {
						scope, 
						callbackURL:`${domain}${req.url.replace(name,`${name}/callback`)}`
					})
				...
				this.on(callbackURL, Object.assign((req, res)=>{
					const authenticate2=conf.passport.authenticate(key, {
						callbackURL:`${domain}${req.url.split("?")[0]}`
					})
					authenticate2(req, res, error=>{
						if(error)
							return
						const url=(()=>{
